...
function addToCart(product) {
    ...
    if (existingItem) {
        ...
        showToast(`📦 ${product.title} (quantity: ${existingItem.quantity})`, 'success');
    } else {
        cart.push({
            id: product.id,
            title: product.title,
...
function removeFromCart(productId) {
    ...
    if (index !== -1) {
        ...
        cart.splice(index, 1);
        showToast(`🗑️ ${removed.title} dihapus dari keranjang`, 'info');
        saveCartToStorage();
        renderCartModal(); // Refresh tampilan keranjang jika terbuka
    }
}
...
function renderCartModal() {
    ...
    if (cart.length === 0) {
        modalBody.innerHTML = `
            <div style="text-align:center; padding:40px;">
                ...
                <br><br>
                <button class="btn-submit" onclick="closeCartModal()" style="margin-top:10px;">Tutup</button>
            </div>
        `;
    ...
    modalBody.innerHTML = cart.map(item => `
        <div style="display:flex; align-items:center; gap:15px; padding:12px; border-bottom:1px solid #eee; margin-bottom:10px;">
            <img src="${getValidImageUrl(item.thumbnail, item.title)}" style="width:60px; height:60px; object-fit:cover; border-radius:8px;">
            <div style="flex:1;">
                <div style="font-weight:bold;">${escapeHtml(item.title)}</div>
                <div style="color:#e94560;">$${item.price}</div>
            </div>
            <div style="display:flex; align-items:center; gap:8px;">
                <button onclick="updateCartQuantity(${item.id}, ${item.quantity - 1})" style="width:30px; height:30px; border-radius:5px; border:1px solid #ddd; background:white; cursor:pointer;">-</button>
                <span style="min-width:30px; text-align:center;">${item.quantity}</span>
                <button onclick="updateCartQuantity(${item.id}, ${item.quantity + 1})" style="width:30px; height:30px; border-radius:5px; border:1px solid #ddd; background:white; cursor:pointer;">+</button>
            </div>
            <button onclick="removeFromCart(${item.id})" style="background:#e94560; color:white; border:none; padding:8px 12px; border-radius:5px; cursor:pointer;">Hapus</button>
        </div>
    `).join('');
...
function checkout() {
    ...
    const total = getCartTotal();
    showToast(`🎉 Checkout berhasil! Total: $${total.toFixed(2)}. Terima kasih!`, 'success');
    cart = [];
    saveCartToStorage();
    renderCartModal();
    closeCartModal();
...
// ========== FUNGSI UNTUK GET GAMBAR YANG VALID ==========
function getValidImageUrl(url, title = 'Product') {
    if (!url || url === '' || url.includes('pin.it') || url.includes('pinterest')) {
        return `https://placehold.co/400x300/0f3460/white?text=${encodeURIComponent(title.substring(0, 25))}`;
    }
    if (!url.startsWith('http')) {
        return `https://placehold.co/400x300/e94560/white?text=Invalid+URL`;
    }
    return url;
}
...
function renderProducts(products) {
    ...
    grid.innerHTML = products.map(p => {
        const validImageUrl = getValidImageUrl(p.thumbnail, p.title);
        ...
        <div class="product-card" data-id="${p.id}">
            <img 
                ...
                onerror="this.src='https://placehold.co/400x300/e94560/white?text=Gambar+Error'"
            >
            <div class="product-info">
                ...
                <div class="product-actions">
                    <button class="add-to-cart-btn" data-id="${p.id}" data-title="${escapeHtml(p.title)}" data-price="${p.price}" data-img="${validImageUrl}">🛒 Beli</button>
...
async function handleAddEditProduct(e) {
    ...
    imageUrl = getValidImageUrl(imageUrl, productName);
...
async function handleDetail(productId) {
    try {
        ...
        document.getElementById('modalBody').innerHTML = `
            ...
            <img 
                src="${validImageUrl}" 
                style="width:100%;max-height:250px;object-fit:contain;margin:15px 0;"
                onerror="this.src='https://placehold.co/400x300/e94560/white?text=Gambar+Error'"
            >
            ...
            <button onclick="addToCart({id:${p.id}, title:'${escapeHtml(p.title)}', price:${p.price}, thumbnail:'${validImageUrl}'})" style="margin-top:20px; width:100%; background:#0f3460; color:white; border:none; padding:12px; border-radius:8px; cursor:pointer;">🛒 Tambah ke Keranjang</button>
...
// ========== EXPOSE GLOBAL FUNCTIONS UNTUK HTML ==========
...
// ========== INIT ==========
function init() {
    console.log('App started - Fitur keranjang aktif!');
    loadCartFromStorage();
    loadCategories();
    loadAll();
    ...
    document.querySelector('.close-form').addEventListener('click', closeFormModal);
    ...
    document.getElementById('productsGrid').addEventListener('click', (e) => {
        ...
        if (addToCartBtn) {
            const product = {
                ...
                thumbnail: addToCartBtn.dataset.img
            };
            addToCart(product);
        } else if (editBtn) {
